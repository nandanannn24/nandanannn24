<div align="center">

<!-- ██████████████████ HERO BANNER ██████████████████ -->
<img src="https://capsule-render.vercel.app/api?type=venom&height=280&text=NandaNannn&fontSize=80&fontColor=00f5ff&color=0:0b0914,50:1e1136,100:0b0914&stroke=9d4edd&strokeWidth=1.5&animation=fadeIn&desc=System%20Architect%20%7C%20Full-Stack%20Developer%20%7C%20AI%20Engineer&descAlignY=75&descSize=16&descColor=b829ea" width="100%"/>

<!-- ██████████████████ TYPING SVG ██████████████████ -->
<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=700&size=22&duration=3000&pause=800&color=00F5FF&center=true&vCenter=true&multiline=false&random=false&width=700&height=50&lines=%3E+INITIALIZING+SYSTEM...;%3E+FULL-STACK+ARCHITECT+AT+UPN+VETERAN+JATIM;%3E+INDEPENDENT+SOFTWARE+ENGINEER;%3E+ARCHITECTING+SCALABLE+WEB+SOLUTIONS;%3E+INTEGRATING+AI+%26+AUTOMATION+PIPELINES;%3E+DELIVERING+HIGH-PERFORMANCE+SYSTEMS" alt="Typing SVG" />

<br/>

<!-- ██████████████████ STATUS PILLS ██████████████████ -->
<img src="https://img.shields.io/badge/STATUS-CODING_LATE_NIGHTS-00f5ff?style=for-the-badge&labelColor=0b0914&logo=statuspage&logoColor=00f5ff"/>
&nbsp;
<img src="https://img.shields.io/badge/MISSION-BUILDING_NEXUS_OMNI-9d4edd?style=for-the-badge&labelColor=0b0914"/>
&nbsp;
<img src="https://img.shields.io/badge/MODE-DARK_THEME_ONLY-1e1136?style=for-the-badge&labelColor=0b0914&logo=github&logoColor=c9d1d9"/>
&nbsp;
<img src="https://komarev.com/ghpvc/?username=nandanannn24&style=for-the-badge&color=9d4edd&label=PROFILE_SCANS&abbreviated=true"/>

</div>

---

<!-- ██████████████████ EXECUTIVE SUMMARY ██████████████████ -->

<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr>
<td width="58%" valign="top">

### `〔 01 〕` ⬡ OPERATOR PROFILE — THE LORE
```yaml### `〔 01 〕` ⬡ OPERATOR PROFILE — THE LORE

```yaml
╔══════════════════════════════════════════════════════╗
║        PROFESSIONAL DOSSIER :: NANDA                 ║
╠══════════════════════════════════════════════════════╣
║  NAME        : Priyoga Listyo Ananda                 ║
║  ROLE        : Full-Stack Architect & AI Engineer    ║
║  INSTITUTION : UPN "Veteran" Jawa Timur              ║
║              : Informatics — Class of 2024           ║
║  CURRENT OBJ : Architecting "SiPeRu" system &        ║
║              : scaling "Nexus Omni" infrastructure.  ║
║  BACKGROUND  : SMKN 3 Buduran TKJ Alum               ║
║  FOCUS       : Scalable Systems & AI Integrations    ║
║  EXPERTISE   : Cloud Architecture & Web Technologies ║
╠══════════════════════════════════════════════════════╣
║  WORK ETHIC  : Precision, Efficiency, Excellence     ║
║  DARK MODE   : System Default = True                 ║
╚══════════════════════════════════════════════════════╝
</td>
<td width="4%"></td>
<td width="38%" valign="top">


### `〔 02 〕` ⬡ DEVELOPMENT UPTIME

<br/>

<img src="https://github-readme-stats-eight-theta.vercel.app/api?username=nandanannn24&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0b0914&title_color=00f5ff&icon_color=9d4edd&text_color=c9d1d9&border_radius=12&include_all_commits=true&rank_icon=github" width="100%"/>

<br/>

<img src="https://github-readme-stats-eight-theta.vercel.app/api/top-langs/?username=nandanannn24&layout=compact&theme=tokyonight&hide_border=true&bg_color=0b0914&title_color=00f5ff&text_color=c9d1d9&border_radius=12&langs_count=8" width="100%"/>

</td>
</tr>
</table>

---

<!-- ██████████████████ STREAK STATS ██████████████████ -->

<div align="center">

### `〔 03 〕` ⬡ COMMIT TELEMETRY

<img src="https://streak-stats.demolab.com?user=nandanannn24&theme=tokyonight&hide_border=true&background=0b0914&ring=00f5ff&fire=9d4edd&currStreakLabel=00f5ff&sideLabels=c9d1d9&dates=b829ea&stroke=9d4edd30&border_radius=12" width="80%"/>

</div>

---

<!-- ██████████████████ TECH STACK MATRIX ██████████████████ -->

### `〔 04 〕` ⬡ ENGINEERING ARSENAL

<table width="100%" border="0" cellspacing="0" cellpadding="10">

<tr>
<td width="50%" valign="top" align="center">

#### ⬡ BACKEND & SYSTEMS ARCHITECTURE

<img src="https://skillicons.dev/icons?i=nodejs,python,laravel,mysql,mongodb,debian,linux&theme=dark&perline=7" />

<br/>
<sub><code>VMware · MikroTik · Advanced Bash Scripting</code></sub>

</td>
<td width="50%" valign="top" align="center">

#### ⬡ FRONTEND & WEB 3D EXPERIENCES

<img src="https://skillicons.dev/icons?i=react,ts,js,tailwind,vite,threejs,figma&theme=dark&perline=7" />

<br/>
<sub><code>WebGL & Premium Minimalist Interfaces</code></sub>

</td>
</tr>

<tr>
<td width="50%" valign="top" align="center">

#### ⬡ AI & AUTOMATION PIPELINES

<img src="https://skillicons.dev/icons?i=tensorflow,python,githubactions,discord&theme=dark&perline=7" />

<br/>
<sub><code>n8n · LLM APIs · Intelligent Workflow Automation</code></sub>

</td>
<td width="50%" valign="top" align="center">

#### ⬡ MULTIMEDIA & CREATIVE PRODUCTION

<img src="https://skillicons.dev/icons?i=blender,ae,pr,figma,ps&theme=dark&perline=7" />

<br/>
<sub><code>Blender · Premiere · After Effects — Creative Direction</code></sub>

</td>
</tr>

<tr>
<td colspan="2" align="center">

#### ⬡ INFRASTRUCTURE & DEVTOOLS

<img src="https://skillicons.dev/icons?i=git,github,docker,vscode,postman,nginx,cloudflare&theme=dark&perline=7" />

</td>
</tr>

</table>

---

<!-- ██████████████████ FEATURED PROJECTS ██████████████████ -->

### `〔 05 〕` ⬡ FEATURED DEPLOYMENTS

<table width="100%" border="0" cellspacing="0" cellpadding="8">

<!-- ROW 1 -->
<tr>
<td width="50%" valign="top">

<a href="https://github.com/nandanannn24">
<img src="https://github-readme-stats.vercel.app/api/pin/?username=nandanannn24&repo=nexus-omni&theme=tokyonight&hide_border=true&bg_color=0b0914&title_color=00f5ff&icon_color=9d4edd&text_color=c9d1d9&border_radius=12" width="100%"/>
</a>

```
┌─── NEXUS OMNI ───────────────────────────────────┐
│  TYPE    : AI Platform + Local Network Hub        │
│  STACK   : Node.js · Python · LLM APIs · n8n     │
│  ROLE    : Lead Architect + Full-Stack Dev        │
│  STATUS  : 🟢 IN DEVELOPMENT                      │
│  DESC    : Intelligent automation platform de-    │
│            signed to optimize complex workflows.  │
│            Aggregating multiple LLMs via n8n      │
│            pipelines to deliver highly scalable   │
│            and efficient AI-driven solutions.     │
└───────────────────────────────────────────────────┘
```

</td>
<td width="50%" valign="top">

<a href="https://github.com/nandanannn24">
<img src="https://github-readme-stats.vercel.app/api/pin/?username=nandanannn24&repo=siperu&theme=tokyonight&hide_border=true&bg_color=0b0914&title_color=00f5ff&icon_color=9d4edd&text_color=c9d1d9&border_radius=12" width="100%"/>
</a>

```
┌─── SiPeRu ───────────────────────────────────────┐
│  TYPE    : Campus Digital Room Reservation System │
│  STACK   : Laravel · MySQL · React · Tailwind     │
│  ROLE    : Full-Stack Dev + Designer              │
│  STATUS  : 🟢 AWAITING CAMPUS APPROVAL             │
│  DESC    : Comprehensive digital room reservation │
│            system for UPN Jatim. Features robust  │
│            scheduling logic, role-based access    │
│            control, and real-time slot management │
│            to streamline academic operations.     │
└───────────────────────────────────────────────────┘
```

</td>
</tr>

<!-- ROW 2 -->
<tr>
<td width="50%" valign="top">

<a href="https://github.com/nandanannn24">
<img src="https://github-readme-stats.vercel.app/api/pin/?username=nandanannn24&repo=kawan_umkm&theme=tokyonight&hide_border=true&bg_color=0b0914&title_color=00f5ff&icon_color=9d4edd&text_color=c9d1d9&border_radius=12" width="100%"/>
</a>

```
┌─── KAWAN UMKM ───────────────────────────────────┐
│  TYPE    : SME Digitalization Platform            │
│  STACK   : Python (BE) · React · TypeScript (FE)  │
│  ROLE    : Full-Stack + Team Lead                 │
│  STATUS  : 🏆 FINALIST MIA 2025 — WEB IN ACTION   │
│  TEAM    : Sekawan Papat                          │
│  DESC    : Empowering SME owners to manage their  │
│            products and access business analytics.│
│            Developed with a Python REST backend   │
│            and a responsive React SPA frontend.   │
│            Recognized in MIA 2025 Web In Action.  │
└───────────────────────────────────────────────────┘
```

</td>
<td width="50%" valign="top">

<a href="https://github.com/nandanannn24">
<img src="https://github-readme-stats.vercel.app/api/pin/?username=nandanannn24&repo=lockerroom-clothing&theme=tokyonight&hide_border=true&bg_color=0b0914&title_color=00f5ff&icon_color=9d4edd&text_color=c9d1d9&border_radius=12" width="100%"/>
</a>

```
┌─── LOCKERROOM CLOTHING + 3D PORTFOLIO ───────────┐
│  TYPE    : E-Commerce + WebGL 3D Portfolio        │
│  STACK   : TypeScript · Three.js · React · Vite  │
│  ROLE    : Dev + 3D Artist + Designer (solo)      │
│  STATUS  : 🟢 LIVE                                 │
│  DESC    : Full-featured TypeScript e-commerce    │
│            platform for TEM Production. Integrates│
│            advanced WebGL and custom Three.js     │
│            shaders to deliver a highly immersive  │
│            and visually immaculate 3D experience. │
└───────────────────────────────────────────────────┘
```

</td>
</tr>

</table>

---

<!-- ██████████████████ ACTIVITY GRAPH ██████████████████ -->

<div align="center">

### `〔 06 〕` ⬡ ACTIVITY TELEMETRY

<img src="https://github-readme-activity-graph.vercel.app/graph?username=nandanannn24&bg_color=0b0914&color=00f5ff&line=9d4edd&point=00f5ff&area=true&area_color=9d4edd&hide_border=true&custom_title=CONSISTENT+SYSTEM+CONTRIBUTIONS&radius=8" width="95%"/>

</div>

---

<!-- ██████████████████ ACHIEVEMENT TROPHIES ██████████████████ -->

<div align="center">

### `〔 07 〕` ⬡ TROPHY CABINET — ACHIEVEMENTS UNLOCKED

<img src="https://github-profile-trophy.vercel.app/?username=nandanannn24&theme=tokyonight&no-frame=true&no-bg=true&margin-w=6&margin-h=6&column=7" width="95%"/>

</div>

---

<!-- ██████████████████ CONTACT ██████████████████ -->

### `〔 08 〕` ⬡ INITIATE HANDSHAKE PROTOCOL

<table width="100%" border="0" cellspacing="0" cellpadding="8">
<tr>
<td width="60%" valign="top">

```bash
# CONTACT PROTOCOL :: TRANSMISSION CHANNELS
$ curl --request COLLAB \
    --url "mailto:priyogalistyoananda26@gmail.com" \
    --header "Subject: [INQUIRY] Project Collaboration" \
    --data "{
      'from'    : 'YOUR_NAME',
      'intent'  : 'COLLABORATION / BUSINESS INQUIRY',
      'note'    : 'Standard response time is within
                   24 business hours.'
    }"
```

> **Open for Opportunities:** Actively seeking roles or contracts 
> involving Full-Stack Development, 3D Web Experiences, and AI Integrations. 
> Dedicated to delivering high-quality, scalable solutions.

</td>
<td width="40%" valign="top" align="center">

<br/>

[![Email](https://img.shields.io/badge/GMAIL-priyogalistyoananda26%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white&labelColor=0b0914)](mailto:priyogalistyoananda26@gmail.com)

<br/>

[![GitHub](https://img.shields.io/badge/GITHUB-nandanannn24-00f5ff?style=for-the-badge&logo=github&logoColor=white&labelColor=0b0914)](https://github.com/nandanannn24)

<br/>

[![Instagram](https://img.shields.io/badge/INSTAGRAM-nandanannn-C13584?style=for-the-badge&logo=instagram&logoColor=white&labelColor=0b0914)](https://instagram.com/nandanannn)

<br/>

![Available](https://img.shields.io/badge/STATUS-OPEN_FOR_WORK-00f5ff?style=for-the-badge&labelColor=0b0914)

</td>
</tr>
</table>

---

<!-- ██████████████████ FOOTER ██████████████████ -->

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&height=120&color=0:0b0914,50:1e1136,100:0b0914&section=footer&text=&fontColor=00f5ff&fontSize=0" width="100%"/>

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=13&duration=4000&pause=1000&color=b829ea&center=true&vCenter=true&width=700&height=30&lines=%22Striving+for+excellence+in+every+commit.%22;%22Building+robust+and+scalable+architectures.%22;%22Code%2C+Deploy%2C+Optimize%2C+Repeat.%22" />

<br/>

<sub>
<code>
⬡ Architected and developed by
<a href="https://github.com/nandanannn24">nandanannn24</a>
· Powered by dedication and clean code.
</code>
</sub>

</div>
